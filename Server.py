...
class Server:
    ...
    def __init__(self):
        ...
        self.clientML = automl.TablesClient(project='undertheradar', region='us-central1')
    ...
    def sentimentValue(self, t):
        document = types.Document(content=t, type=enums.Document.Type.PLAIN_TEXT, language="en")
        score = self.client.analyze_sentiment(document).document_sentiment.score
        text = list()
        text.append(score)
        i = 0;
        for e in self.client.analyze_entity_sentiment(document).entities:
            if (i > 2):
                break
    ...
    def predict(self, inputs):
        response = server.clientML.predict(
